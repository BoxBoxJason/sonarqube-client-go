...
var _ = Describe("Qualitygates Service", Ordered, func() {
	...
	Describe("AddGroup", func() {
		It("should add group permission to quality gate", func() {
			...
			for _, group := range groupsResult.Groups {
				if group.Name == "sonar-users" {
					foundGroup = true
					break
				}
			}
			Expect(foundGroup).To(BeTrue(), "Group 'sonar-users' should have permission on quality gate")
		})
	...
	Describe("RemoveGroup", func() {
		It("should remove group permission from quality gate", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			for _, group := range groupsResult.Groups {
				Expect(group.Name).NotTo(Equal("sonar-users"), "Group 'sonar-users' should not have permission on quality gate after removal")
			}
		})
