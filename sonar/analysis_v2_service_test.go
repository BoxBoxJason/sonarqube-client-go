...
func TestAnalysisV2_DownloadJre_Validation(t *testing.T) {
	...
	var buf bytes.Buffer
	_, err := client.V2.Analysis.DownloadJre("", &buf)
	assert.Error(t, err)
}
...
func TestAnalysisV2_GetActiveRules(t *testing.T) {
	...
	result, resp, err := client.V2.Analysis.GetActiveRules(&AnalysisActiveRuleV2sOptions{
...
func TestAnalysisV2_GetActiveRules_Validation(t *testing.T) {
	...
	tests := []struct {
		name string
		opt  *AnalysisActiveRuleV2sOptions
	}{
		{"nil opt", nil},
		{"missing project key", &AnalysisActiveRuleV2sOptions{}},
	}
